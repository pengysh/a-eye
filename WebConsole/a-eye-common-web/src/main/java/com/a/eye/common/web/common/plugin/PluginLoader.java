package com.a.eye.common.web.common.plugin;
...
/**
 * TODO Loading all plugins with a single class loader could cause class
 * collisions. Also, with current implementation, plugins can use dependencies
 * by putting them in the plugin directory too. But it can lead to dependency
 * collision between plugins because they are loaded by a single class loader.
 * 
 * How can we prevent this? A ClassLoader per plugin could do it but then we
 * have to create "N of target class loader" x "N of plugin" class loaders. It
 * seems too much. For now, Just leave it as it is.
 * 
 * 
